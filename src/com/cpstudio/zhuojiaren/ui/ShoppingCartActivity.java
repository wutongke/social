...
public class ShoppingCartActivity extends Activity {
			...
			ShoppingCartActivity.this.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					if (result.equals(BCPayResult.RESULT_SUCCESS)) {
						...
						String toastMsg = "支付失败, 原因: "
								+ bcPayResult.getErrCode() + " # "
								+ bcPayResult.getErrMsg() + " # "
								+ bcPayResult.getDetailInfo();

						if (bcPayResult.getErrMsg()
								.equals("PAY_FACTOR_NOT_SET")
								&& bcPayResult.getDetailInfo().startsWith(
										"支付宝参数")) {
						...
						Toast.makeText(ShoppingCartActivity.this, toastMsg,
								Toast.LENGTH_LONG).show();
						Log.e(TAG, toastMsg);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String initInfo = BCPay.initWechatPay(ShoppingCartActivity.this,
				"wx95c80614da3d5a6a");
		if (initInfo != null) {
		...
		Integer[] payIcons = new Integer[] { R.drawable.wechat,
				R.drawable.alipay };
		final String[] payNames = new String[] { "微信支付", "支付宝支付" };
		String[] payDescs = new String[] { "使用微信支付，以人民币CNY计费",
				"使用支付宝支付，以人民币CNY计费" };
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				case 0: // 微信
					...
					if (BCPay.isWXAppInstalledAndSupported()
						...
						Toast.makeText(ShoppingCartActivity.this,
								"您尚未安装微信或者安装的微信版本不支持", Toast.LENGTH_LONG)
								.show();
						loadingDialog.dismiss();
					}
					break;
				...
				case 1: // 支付宝支付
					...
					BCPay.getInstance(ShoppingCartActivity.this)
							.reqAliPaymentAsync("倬币支付", money, // 订单金额(分)
									number, mapOptional, bcCallback);
				...
				case 2: // 银联支付
					 ...
					 * BCPay.getInstance(ShoppingCartActivity
					 * .this).reqUnionPaymentAsync("银联支付测试", 1,
					 * BillUtils.genBillNum(), null, bcCallback);
					 */
				...
				case 3: // 通过百度钱包支付
					...
					/*
					 * 支付渠道，此处以百度钱包为例，实际支付允许 BCReqParams.BCChannelTypes.WX_APP，
				...
				case 4: // 通过PayPal支付
					...
					bcPay.addPayPalSyncObserver(new BCPayPalSyncObserver() {
						...
						@Override
						public boolean onSyncFailed(String billInfo,
								String failInfo) {
							Log.w(TAG, "billInfo: " + billInfo + "#failInfo: "
									+ failInfo);
							// return true if you have successfully dealt with
							// the billInfo
							// else sdk will continue to store the un-synced
							// billInfo into cache for later sync
							return false;
						}
					});

					BCPay.getInstance(ShoppingCartActivity.this)
							.reqPayPalPaymentAsync("PayPal payment test", // bill
																			// title
	...
	void getBillInfoByID(String id) {
		...
		BCQuery.getInstance().queryBillByIDAsync(id, new BCCallback() {
			...
			public void done(BCResult result) {
				...
				Log.d(TAG,
						"------getResultCode------"
								+ billResult.getResultCode());
				Log.d(TAG,
						"------getResultMsg------" + billResult.getResultMsg());
				Log.d(TAG,
						"------getErrDetail------" + billResult.getErrDetail());
				...
				Log.d(TAG,
						"渠道类型:"
								+ BCReqParams.BCChannelTypes
										.getTranslatedChannelName(billOrder
												.getChannel()));
				Log.d(TAG,
						"子渠道类型:"
								+ BCReqParams.BCChannelTypes
				...
				if (billOrder.getPayResult())
					Log.d(TAG,
							"渠道返回的交易号，未支付成功时，是不含该参数的:"
									+ billOrder.getTradeNum());
				else
					Log.d(TAG,
							"订单是否被撤销，该参数仅在线下产品（例如二维码和扫码支付）有效:"
