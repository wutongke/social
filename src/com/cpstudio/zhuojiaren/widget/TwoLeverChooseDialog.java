...
public class TwoLeverChooseDialog extends AlertDialog {
	...
	private int[] itemIds;// = {R.array.beijin_province_item,
	...
	public TwoLeverChooseDialog(Context context, int theme,
			...
			int firstLeverId, int[] secondLecerIds) {
		super(context, theme);
	...
	public TwoLeverChooseDialog(Context context, int theme,
			String defaultFirstLeverItem, String defaultSecondLeverItem,
			int firstLeverId, List<List<String>> subStrings) {
		super(context, theme);
		this.mContext = context;
		// this.itemIds=secondLecerIds;
		// TODO Auto-generated constructor stub
	...
	public void setPlace(String mypro, String mycity) {
		int provinceIndex = 0;
		int cityIndex = 0;
		for (provinceIndex = 0; provinceIndex < province.length
				&& mypro != null; provinceIndex++) {
			if (province[provinceIndex].equals(mypro))
				break;
		}
		if (provinceIndex >= province.length)
			provinceIndex = 0;
		for (; cityIndex < itemArray[provinceIndex].length && mycity != null; cityIndex++) {
			if (mycity.equals(itemArray[provinceIndex][cityIndex]))
				break;
		}
		if (cityIndex >= itemArray[provinceIndex].length)
			cityIndex = 0;
		firstLeverWV.setViewAdapter(new ArrayWheelAdapter<String>(mContext,
				province));
		firstLeverWV.setCurrentItem(provinceIndex);
		secondLeverWV.setViewAdapter(new ArrayWheelAdapter<String>(mContext,
				itemArray[provinceIndex]));
